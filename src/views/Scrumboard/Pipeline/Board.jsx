  ...
  return (
    <>
      <Draggable draggableId={props.card.id} index={props.index}>
        {(provided) => (
          <Card className='card-simple card-flush spipeline-list' ref={provided.innerRef} {...provided.draggableProps} id={props.card.id}>
            <Card.Header className='card-header-action' {...provided.dragHandleProps}>
                ...
                <div className='card-action-wrap'>
                  <Dropdown as='a'>
                    ...
                    <Dropdown.Menu align='end'>
                      <Dropdown.Item className='edit-tasklist' onClick={handleRename}>
                        Edit
                      </Dropdown.Item>
                      <Dropdown.Item className='delete-tasklist' onClick={() => props.onRemoveBoard(props.card.id)}>
                        Delete
                      </Dropdown.Item>
                      <Dropdown.Item className='clear-tasklist' onClick={() => props.clearBoard(props.card.id)}>
                        Clear All
                      </Dropdown.Item>
                    </Dropdown.Menu>
                  </Dropdown>
              ...
              <div>
                <span>
                  ...
                  <span className='lead-count'>{props.card.taskIds.length} Leads</span>
                </span>
              </div>
                  ...
                  <>
                    ...
                    >
                      {props.tasks.map((task, index) => (
                        <Task key={task.id} task={task} index={index} cardId={props.card.id} onRemoveTask={props.onRemoveTask} />
                      ))}
                    </div>
      ...
      <Modal Modal show={renameBoard} onHide={() => setRenameBoard(!renameBoard)} size='sm' centered>
        ...
        <div className='modal-footer align-items-center'>
          <Button variant='secondary' onClick={() => setRenameBoard(!renameBoard)}>
            Cancel
          </Button>
          <Button variant='primary' className='btn-add-tasklist' onClick={onRenameBoard}>
            Add
          </Button>
        </div>
      </Modal>
      ...
      <Modal show={addNewCard} onHide={() => setAddNewCard(!addNewCard)} centered>
        <div className='modal-content'>
          <Modal.Body>
            ...
            <Form>
              <Row className='gx-3'>
                ...
                <Col sm={12}>
                  <Form.Group className='mb-3'>
                    <HkDropZone className='dropify-square w-30'>Upload Photo</HkDropZone>
                  </Form.Group>
                </Col>
                <Col sm={12}>
                  <div className='form-inline'>
                    <Form.Group className='mt-3'>
                      ...
                      <Form.Check inline label='High' name='group1' type='radio' id={'inline-1'} defaultChecked />
                      <Form.Check inline label='Medium' name='group1' type='radio' id={'inline-2'} />
                      <Form.Check inline label='Low' name='group1' type='radio' id={'inline-2'} />
                    </Form.Group>
                  </div>
                </Col>
              </Row>
          ...
          <Modal.Footer className='align-items-center'>
            <Button variant='secondary' onClick={() => setAddNewCard(!addNewCard)}>
              Cancel
            </Button>
            <Button variant='primary' className='btn-add-task' onClick={onSaveTask}>
              Add
            </Button>
          </Modal.Footer>
        </div>
