      ...
      <Draggable draggableId={props.task.id} index={props.index}>
        {(provided, snapshot) => (
          ...
          >
            ...
            {(props.task.Avatar || props.task.Badges || props.task.Description) && (
              <Card.Body>
                {props.task.Description && <p>{props.task.Description}</p>}
                {(props.task.Avatar || props.task.InitAvatar) && (
                  <div className='avatar-group avatar-group-overlapped'>
                    {props.task.Avatar &&
                      props.task.Avatar.map((ele, indx) => (
                        <HkTooltip placement='top' title={ele.name}>
                          <div className='avatar avatar-rounded' key={indx}>
                            <img src={ele.img} alt='user' className='avatar-img' />
                          </div>
                        </HkTooltip>
                      ))}
                    {props.task.InitAvatar &&
                      props.task.InitAvatar.map((elem, ind) => (
                        <HkTooltip placement='top' title={elem.name}>
                ...
                {props.task.Badges && (
                  <div className='mt-2'>
                    {props.task.Badges.map((badge, ind) => (
                      <HkBadge bg={badge.bg} soft className='my-1 me-1' key={ind}>
                        {badge.title}
                      </HkBadge>
                    ))}
                  </div>
            ...
            {props.task.Footer && (
              <Card.Footer className='text-muted justify-content-between'>
                ...
                <div>
                  <span className='task-deadline'>{props.task.Deadline}</span>
                </div>
              </Card.Footer>
            )}
          </Card>
