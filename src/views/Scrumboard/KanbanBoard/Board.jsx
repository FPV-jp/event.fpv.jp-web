...
const Board = () => {
  ...
  const [dataset, setDataset] = useState(() => {
    const savedDataset = sessionStorage.getItem('kanban-board')
  ...
  useEffect(() => {
    sessionStorage.setItem('kanban-board', JSON.stringify({ tasks, cards, cardOrder }))
    ...
    <>
        ...
        <div>
          <div className='taskbar-toolbar'>
            <div className='d-flex align-items-center flex-grow-1 flex-lg-grow-0'>
              <Button variant='soft-primary' className='flex-shrink-0 btn-add-newlist me-4' onClick={() => setAddNewBoard(!addNewBoard)}>
                Create New
              </Button>
              <Form.Check type='switch' id='custom-switch' label='Show description' className='ms-auto' defaultChecked />
            </div>
          ...
          <PerfectScrollbar className='tasklist-scroll position-relative'>
            <div id='tasklist_wrap'>
              <DragDropCards cards={cards} tasks={tasks} cardOrder={cardOrder} setCards={setCards} setTasks={setTasks} setCardOrder={setCardOrder} />
            </div>
            <div className='card card-simple card-border tasklist add-new-task'>
              <Button variant='soft-primary' className='btn-add-newlist flex-shrink-0' onClick={() => setAddNewBoard(!addNewBoard)}>
                Add New List
              </Button>
            </div>
          </PerfectScrollbar>
      ...
      <Modal show={addNewBoard} onHide={() => setAddNewBoard(!addNewBoard)} size='sm' centered className='add-tasklist-modal'>
        ...
        <Modal.Footer className='align-items-center'>
          <Button variant='secondary' onClick={() => setAddNewBoard(!addNewBoard)}>
            Cancel
          </Button>
          <Button variant='primary' className='btn-add-Board' onClick={onAddNewCard}>
            Add
          </Button>
        </Modal.Footer>
      </Modal>
