...
const Footer = ({ groupMessage, grpMsg }) => {
  ...
  //Send a new messages
  const sendMessage = () => {
    groupMessage({ text: message, time: msgTitme, types: 'sent' })
    if (grpMsg) {
      setTimeout(() => {
        groupMessage({ text: 'What are you saying?', time: msgTitme, types: 'received' })
      }, 800)
    }
  }
  const handleClick = () => {
...
export default connect(mapStateToProps, { groupMessage })(Footer)
