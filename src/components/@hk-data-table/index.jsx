    ...
    <>
      {searchBar && !searchQuery && (
        <Form.Group controlId='searchForm' className='mb-3'>
          <Form.Control type='search' placeholder='Search' value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className={searchClasses} />
        </Form.Group>
      )}
      <Table bsPrefix={bsPrefix} className={classNames('hk-data-table', classes)} striped={striped} bordered={bordered} borderless={borderless} hover={hover} size={size} variant={variant} responsive={responsive} {...rest}>
        <thead>
          <tr>
            {(rowSelection || markStarred) && <th>{rowSelection ? <Form.Check type='checkbox' className='fs-6 mb-0' checked={selectAll} onChange={handleSelectAll} /> : <></>}</th>}
            {column.map((cols, index) => (
              <th key={index} onClick={() => requestSort(cols.sort, cols.accessor)} className={classNames({ 'd-none': cols.hidden }, { 'text-primary': sortConfig !== null && sortConfig.key === cols.accessor }, cols.className)} rowSpan={cols.rowSpan}>
        ...
        <tbody>
          ...
          {filteredData.map((row, index) => (
            <tr key={index} className={classNames({ selected: isRowSelected(index) })}>
              {(rowSelection || markStarred) && (
                <td>
                  <div className='d-flex align-items-center'>
                    {rowSelection && <Form.Check type='checkbox' className='form-check fs-6 mb-0' checked={isRowSelected(index)} onChange={() => handleRowSelection(index)} />}
                    {markStarred && (
                      <span className={classNames('fav-star', { marked: row.starred })} onClick={() => handleStared(index)}>
              ...
              {column.map((cols, index) => (
                <td key={index} className={classNames({ 'd-none': cols.hidden }, cols.tdClasses)}>
                  {cols.cellFormatter ? cols.cellFormatter(row[cols.accessor]) : row[cols.accessor]}
                </td>
              ))}
            </tr>
          ))}
      ...
      {rowsPerPage && <TableFooter range={range} slice={slice} setPage={setPage} page={page} totalRows={data} paginatorSize={paginatorSize} />}
    </>
  )
}
